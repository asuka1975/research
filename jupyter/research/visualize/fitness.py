...
class Fitness:
    def __new__(cls, trial: Trial, ymin=None, ymax=None, extras={}):
        ...
        f = trial.fitness()
        self = figure(title="fitness", width=500, height=400)
        ...
        self.line(list(range(len(f["maxes"]))), [v * (fmax - fmin) + fmin for v in f["maxes"]], color=c[0], legend_label='max')
        self.line(list(range(len(f["maxes"]))), [v * (fmax - fmin) + fmin for v in f["means"]], color=c[1], legend_label='mean')
